...
int main()
{
	...
	{
		using my_algo_t = mr_crypt::supreme::aria_128_cbc;
		const auto keys = my_algo_t::make_key();

		// Stateless way
		{
			// Way 01
			{
				const auto encrypt = my_data | my_algo_t::encrypt_t(keys);
				const auto decrypt = encrypt | my_algo_t::decrypt_t(keys);
			}
			// Way 02
			{
				const auto encrypt = my_data | my_algo_t(keys).encrypt;
				const auto decrypt = encrypt | my_algo_t(keys).decrypt;
			}
		}
		// Stateful way
		{
			const auto my_algo = my_algo_t{ keys };
			// Way 01
			{
				const auto encrypt = my_data | my_algo.encrypt;
				const auto decrypt = encrypt | my_algo.decrypt;
			}
			// Way 02
			{
				const auto encrypt = my_data | my_algo;
				const auto decrypt = encrypt | my_algo.decrypt;
			}
		}

		const auto nice = true;
	}
}
