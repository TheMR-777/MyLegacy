...
namespace mr_crypt
{
	...
	namespace details
	{
		...
		template <const EVP_CIPHER* (*evp_cipher_x)(), bool to_encrypt>
		struct internal_adapter_f : std::ranges::range_adaptor_closure<internal_adapter_f<evp_cipher_x, to_encrypt>>
		{
			...
			{
				return mr_crypt::details::cipher<evp_cipher_x, to_encrypt>(input, my_key, the_iv);
			}
		};
		...
		struct cipher_s
		{
			const std::string my_key = mr_crypt::generate::key(EVP_CIPHER_key_length(evp_cipher_x()));
			const std::string the_iv = mr_crypt::generate::key(EVP_CIPHER_iv_length(evp_cipher_x()));

			auto encrypt() const noexcept
			{
				return internal_adapter_f<evp_cipher_x, true>
				{
					.my_key = my_key,
					.the_iv = the_iv,
				};
			...
			{
				return internal_adapter_f<evp_cipher_x, false>
				{
					.my_key = my_key,
					.the_iv = the_iv,
				};
			...
			auto encrypt(std::string_view input) const noexcept
			{
				return mr_crypt::details::cipher<evp_cipher_x, true>(input, my_key, the_iv);
			};

			auto decrypt(std::string_view input) const noexcept
			{
				return mr_crypt::details::cipher<evp_cipher_x, false>(input, my_key, the_iv);
			}
		};
	...
	namespace ography
	{
		using aes_256_cbc = details::cipher_s<EVP_aes_256_cbc>;
	}

	namespace encrypt
	{
		using aes_256_cbc = details::internal_adapter_f<EVP_aes_256_cbc, true>;
	}

	namespace decrypt
	{
		using aes_256_cbc = details::internal_adapter_f<EVP_aes_256_cbc, false>;
	}
...
int main()
{
	...
	// Stateless
	{
		const auto my_key = mr_crypt::generate::key(32);
		const auto the_iv = mr_crypt::generate::key(16);

		const auto enc = my_data | mr_crypt::encrypt::aes_256_cbc
		{
			.my_key = my_key,
			.the_iv = the_iv,
		};

		const auto dec = enc | mr_crypt::decrypt::aes_256_cbc
		{
			.my_key = my_key,
			.the_iv = the_iv,
		};
	...
	// Stateful
	{
		const auto aes = mr_crypt::ography::aes_256_cbc{};

		const auto enc = my_data | aes.encrypt();
		const auto dec = enc | aes.decrypt();
