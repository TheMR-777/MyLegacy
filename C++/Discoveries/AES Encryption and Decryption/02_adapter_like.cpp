...
struct cipher_s
{
	...
	template <bool to_encrypt>
	struct internal_adapter_f : std::ranges::range_adaptor_closure<internal_adapter_f<to_encrypt>>
	{
	...
	{
		return internal_adapter_f<true>
		{
			.my_key = my_key,
			.the_iv = the_iv,
		};
	...
	{
		return internal_adapter_f<false>
		{
			.my_key = my_key,
			.the_iv = the_iv,
		};
...
int main()
{
	...
	using my_cipher = cipher_s<EVP_aes_256_cbc>;

	const auto aes_256_cbc = my_cipher{};
	const auto m_encrypted = my_data | aes_256_cbc.encrypt();
	const auto m_decrypted = m_encrypted | aes_256_cbc.decrypt();

	std::println("Original  : {}", my_data);
	std::println("Encrypted : {}", m_encrypted | mr_crypt::convert::to_hex);
	std::println("Decrypted : {}", m_decrypted);
}
