...
namespace crypt 
{
	...
	// From std::string_view of Base64
	auto from_base64(std::string_view base64_encoded) 
	{
		auto d_len = int{};
		auto decoded = std::string(base64_encoded.size(), '\0');

		const auto ctx = EVP_ENCODE_CTX_new();
		EVP_DecodeInit(ctx);
		EVP_DecodeUpdate(
			ctx, 
			reinterpret_cast<value_type*>(decoded.data()),
			&d_len, 
			reinterpret_cast<const value_type*>(base64_encoded.data()),
			base64_encoded.size()
		);
		EVP_DecodeFinal(
			ctx, 
			reinterpret_cast<value_type*>(decoded.data()) + d_len,
			&d_len
		);
