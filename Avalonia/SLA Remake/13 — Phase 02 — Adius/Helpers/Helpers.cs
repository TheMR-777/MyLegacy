...
public static class CrossUtility
{
	...
	public static void RecordAndStreamAudioData()
	{
		...
		var output = method?.StandardError.ReadToEnd() ?? string.Empty;
		var simile = LowLevel_APIs.AudioInputDevice_FFMPEG().Match(output);

		var source = simile.Success ? simile.Groups[1].Value : string.Empty;
#endif

		// Setting the FFMPEG Mode
		// -----------------------

		var ffmpegMode =
#if WIN
			$"dshow -i audio={source}";
#elif MAC
...
public static partial class LowLevel_APIs
{
	...
	public static partial Regex AudioInputDevice_FFMPEG();
