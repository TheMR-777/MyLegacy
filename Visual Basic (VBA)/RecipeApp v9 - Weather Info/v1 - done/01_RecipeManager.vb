Public Class RecipeApp
	...
	Private Sub Load_Online()
		...
		Try
			...
			Dim RawLocale = Client.DownloadString(Locale_URL)
			Locale = Newtonsoft.Json.JsonConvert.DeserializeObject(Of Dictionary(Of String, String))(RawLocale)

			' Load and parse weather data
			Dim RawData = Client.DownloadString(WeatherURL & "?lat=" & Locale("lat") & "&lon=" & Locale("lon") & "&appid=" + WeatherAPI)
			OpenWeather = Newtonsoft.Json.Linq.JObject.Parse(RawData)
		...
		Dim TimeZone = If(Locale.ContainsKey("timezone"), Locale("timezone"), TimeZoneInfo.Local.DisplayName)
		Me.Locale.Text = $"IP: {IPAddress}   |   Timezone: {TimeZone}"
