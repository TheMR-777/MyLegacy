...
namespace Project
{
	class Program
	{
        ...
        public static void Main()
		{
			...
			try
			{
                jwt = Get.JWT(jwt_src, false);
            }
            catch (Exception ex)
			{
				Console.WriteLine("-----Failed to Get JWT Token-----");
				Console.WriteLine();
			...
			var aud_verified = aud == req_aud;
			if (!aud_verified)
			{
                Console.WriteLine("-----JWT Token is not for this Audience-----");
				Console.WriteLine($"Required Audience: {req_aud}");
				Console.WriteLine($"Recieved Audience: {aud}");
                return;
            }
			...
			var sig_verified = new X509Certificate2(Convert.FromBase64String(key))
				.GetRSAPublicKey()!
				.VerifyData(
					data: formatted,
					signature: signature,
			...
			if (!sig_verified)
			{
                Console.WriteLine("-----JWT Token Signature is not Verified-----");
                return;
            }

			Console.WriteLine($"Expiration Verification : {exp_verified}");
			Console.WriteLine($"Audience   Verification : {aud_verified}");
			Console.WriteLine($"Signature  Verification : {sig_verified}");
			Console.WriteLine('\n');
