...
namespace MyPlayground
{
    internal class Program
    {
        private static void Main()
        {
            ...
            var table = CreateTimeTable(prayers, timings);
            var header = $"**Timetable of Prayers — {DateTime.Now:dd/MM/yyyy}**";
        ...
        {
            var header = "Namaz: | " + string.Join(" | ", prayers.Select(p => ConvertZuhr(p).PadRight(8))) + " |";
        ...
        {
            if (prayer == "Fajr") return string.Empty.PadRight(8);
            if (prayer == "Dhuhr") return "01:30 PM";
            ...
            var quarterHour = (minutes / 15) * 15;
            if (minutes % 15 != 0) quarterHour += 15;
            jamatTime = new TimeSpan(jamatTime.Hours, quarterHour, 0);
