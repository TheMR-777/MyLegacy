...
public static class Database<T>
{
    ...
    public static readonly string MyName = AppDomain.CurrentDomain.FriendlyName;
    public static readonly string MyPath = AppDomain.CurrentDomain.BaseDirectory;
    private static readonly string DatabaseLocation = Path.Combine(MyPath, MyName + ".sqlite");
    private static readonly string ConnectionString = $"Data Source={DatabaseLocation};";
    //private static readonly object UltimateLockdown = new();
    private static readonly Microsoft.Data.Sqlite.SqliteConnection Connection = new(ConnectionString);

    private static void EnsureTableExists()
        => Connection.Execute(QueryBuilder.GenerateTable());
    ...
    public static int Save(IEnumerable<T> entries)
    {
        lock (Connection)
        {
            ...
            var query = QueryBuilder.InsertEntity();
...
public static class HeavyTest
{
    ...
    private static class Program
    {
        ...
        private static void TestConcurrentWrites()
        {
            ...
            Console.WriteLine($"Expected entries : {threads_n}");
            Console.WriteLine($"Yielded  entries : {savedEntries.Count}");
        }
